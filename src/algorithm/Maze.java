...
public class Maze {
	 ...
	 *
	 * @param mapData
	 *            The string of data representing the map.
	 * @throws InvalidMazeException
	 *             If there is an error parsing the map.
	 */
	 ...
	 * <p>
	 * This algorithm first visits the starting position by pushing it in to the
	 * queue. Every loop, the first cell in the queue will be popped out and
	 * visited. For every cell that it visits, it will first check if it is the
	 * ending point. If the ending point has been found, the loop is stopped. This
	 * is the shortest path because the points are visited in order of distance away
	 * of the starting point. If there was a shorter path to the ending point, it
	 * would have already been discovered before the current one. If the current
	 * point is not the ending point, it will check all four of its neighbours (up,
	 * down, left, right). It will add all neighbouring cells that are within the
	 * bounds of the map, have not been visited, and are not walls to the end of the
	 * queue. For each of these visitable neighbours, it will set itself as the
	 * parent cell of its neighbour and record the direction from which it reached
	 * the neighbouring cell in the parentArray. This will continue until either the
	 * queue is empty or a goal has been found. If the queue empties before the goal
	 * has been found, it means that the algorithm was unable to find a path from
	 * the starting point to the ending point. If a goal has been found, it will
	 * start with the ending point and trace backwards through the map to find the
	 * route it took using the information stored in the parent array. Once the
	 * starting point is reached (the parent of this point in the parentArray would
	 * be null), the entire path is reversed, an iterator for the path is created,
	 * and the path object returned.
	 *
	...
	public Path solveMaze() {
		...
		// keep track of which points have been visited, and if visited, its parent and
		// the direction that led to the point
